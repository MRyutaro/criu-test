...
CONTAINER_NAME="criu_test"
IMAGE_NAME="localhost/criu_test:latest"
...
echo -e "${BLUE}=== criuテストスクリプト（コンテナ版） ===${NC}"
...
case "$1" in
    ...
    checkpoint)
        ...
        # --tcp-established: アクティブなTCP接続を含める（ネットワーク接続がある場合に必要）
    ...
    restore)
        ...
        # --tcp-established: アクティブなTCP接続を復元（チェックポイント時に--tcp-establishedを使用した場合に必要）
