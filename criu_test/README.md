...
このプロジェクトは、Podmanコンテナでcriu（Checkpoint/Restore In Userspace）を試すための簡単なサンプルです。
...
これにより、Pythonアプリケーションを含むPodmanイメージが作成されます。
...
これにより、カウンターを表示し続けるPythonプロセスがコンテナ内で起動します。

コンテナのログを確認するには：
```bash
sudo podman logs criu_test
```
...
これにより、実行中のコンテナ内のプロセスの状態がチェックポイントディレクトリに保存されます。
...
これにより、チェックポイントからコンテナが復元され、カウンターが続きから再開されます。
...
### チェックポイントに失敗する場合
- コンテナが実行中であることを確認（`podman ps`）
- コンテナのログを確認（`sudo podman logs criu_test`）
- チェックポイントディレクトリ内のログファイルを確認
- カーネルがcriuをサポートしているか確認
- コンテナが`--privileged`モードで起動されているか確認

### コンテナが起動しない場合
- Podmanイメージがビルドされているか確認（`podman images`）
- Podmanが正しく設定されているか確認（`podman info`）
